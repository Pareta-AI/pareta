...
`match` resolves a free-text description of your dataset to the task an eval scores it with — how it's scored (or a general lane, or no match — a statement about scoring, not serving). `--top-k` (default 5) controls how many candidate tasks it considers.
...
pareta evals run --prompt "extract the key fields from each contract" --file rows.jsonl \
  --models auto --frontier --wait

# Or run an existing eval set:
pareta evals run --eval-set es_abc --models auto --wait

# Preview how your rows would be scored (nothing is created):
pareta evals propose --file rows.jsonl --prompt "extract the key fields from each contract"

# Manage eval sets (your data rows):
pareta evals sets create --prompt "extract the key fields from each contract" --file rows.jsonl
pareta evals sets list
pareta evals sets show es_abc
pareta evals sets delete es_abc --yes
```

`--models` is required — pass `auto` to benchmark Pareta's routing itself. When you build a set from `--file`, `--prompt` (one sentence on what the model should do with each item) is required — Pareta works out the scoring from it and your data's shape, so `--task` is optional (pass it only to pin a specific task). Running an existing set with `--eval-set` needs neither. `--frontier` adds the task's benchmarked vendor models as baselines, which is the comparison that matters. Each item in `--file` is one JSON object per line; eval runs are metered against your org balance.
...
pareta evals run --prompt "extract the key fields from each contract" --file rows.jsonl --models auto --frontier --wait
